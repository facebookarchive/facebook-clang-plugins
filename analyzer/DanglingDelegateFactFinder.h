...
namespace DanglingDelegate {
  ...
  public:
    ...
    struct IvarFacts {
      ...
      StringSet _mayStoreSelfInUnsafeProperty; // e.g. we have seen [_x setDelegate:self] in the code where 'delegate' is an assign property
    ...
    struct SharedObserverFacts {
      ...
      StringSet _mayAddObserverToSelfInObjCMethod; // e.g. we have seen [[NSNotificationCenter defaultCenter] addObserver:self ..] in method 'init'
    ...
    struct ObjCImplFacts {
      ...
      typedef std::map<const clang::ObjCIvarDecl *, struct IvarFacts> IvarFactsMap;
      typedef std::map<const std::string, struct SharedObserverFacts> SharedObserverFactsMap;
      ...
      void ivarMayStoreSelfInUnsafeProperty(const clang::ObjCIvarDecl *ivarDecl, const std::string &propName);

      void ivarMayTargetSelf(const clang::ObjCIvarDecl *ivarDecl);

      void ivarMayObserveSelf(const clang::ObjCIvarDecl *ivarDecl);

      void sharedObjectMayObserveSelfInMethod(const std::string objectName, const std::string &methodName);
  ...
  private:
    ...
    // pointer to the structure recording facts over the current class implementation
    struct ObjCImplFacts &_facts;

  public:
    FactFinder(const clang::ObjCMethodDecl *methDecl, struct ObjCImplFacts &facts) : _methDecl(methDecl), _facts(facts) { }
