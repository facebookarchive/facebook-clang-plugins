...
namespace {
  // Stmt walker class: visit all the sub-statements recursively and check If statements
  ...
  public:
    WalkAST(const CheckerBase &checker, BugReporter &br, AnalysisDeclContext* ac, const Decl *decl) :
    _checker(checker), _br(br), _ac(ac), _decl(decl) {}
    
    // Check an If statement: this is where the main logic of the checker is implemented.
  ...
  private:
    // Report the warning.
    void reportWarning(const ObjCMessageExpr &msgExpr, const ValueDecl &receiver, const Selector &sel1, const Selector &sel2) const;
    
    // Check a statement in the body of the 'then' branch until the first candidate call is found.
    bool checkThenBodyStmt(const Stmt *thenBodyStmt, const Expr &condReceiver, const Selector &condSel) const;
    
    // Check a toplevel statement in the body of the 'then' branch.
    void checkToplevelThenBodyStmt(const Stmt &thenBodyS, const Expr &condReceiver, const Selector &condSel) const;
  };
  ...
  static bool isRespondsToSelector(const Selector &sel) {
    ...
    if (sel.getNumArgs() != 1) {
      assert(false); // Check Objective C convention that respondsToSelector has 1 argument.
      return false;
    }
    return true;
  }
  ...
  // If an expression refers to a value, return a reference to its declaration, otherwise NULL.
  ...
  static bool msgExprInstanceMethodExists(const ObjCMessageExpr &msgExpr, const Selector &sel) {
  ...
  // Get a normalized selector from a message expression by stripping 'performSelector' if present.
  static const Selector getNormalizedSelector(const ObjCMessageExpr &msgExpr) {
    ...
    if (isPerformSelector(sel)) {
      ...
      if (const ObjCSelectorExpr *selExpr = dyn_cast_or_null<ObjCSelectorExpr>(selArgument)) {
  ...
  void WalkAST::reportWarning(const ObjCMessageExpr &msgExpr, const ValueDecl &receiver, const Selector &sel1, const Selector &sel2) const {
    auto bugName = "Suspicious use of respondsToSelector";
    auto bugCategory = "Semantic issue (Facebook)";
    
    llvm::SmallString<128> buf;
    llvm::raw_svector_ostream os(buf);
    os << "Suspicious use of respondsToSelector. A condition checks if " << receiver;;
  ...
  bool WalkAST::checkThenBodyStmt(const Stmt *thenBodyStmt, const Expr &condReceiver, const Selector &condSel) const {
    ...
    if (const ObjCMessageExpr *thenBodyMsgExpr = dyn_cast<ObjCMessageExpr>(thenBodyStmt)) {
      ...
      bool isSelKnown = (thenBodySel == condSel) || msgExprInstanceMethodExists(*thenBodyMsgExpr, thenBodySel);
    ...
    } else if (const UnaryOperator *unOp = dyn_cast<UnaryOperator>(thenBodyStmt)) {
      if (checkThenBodyStmt(unOp->getSubExpr(), condReceiver, condSel))
        return true;
    } else if (const BinaryOperator *binOp = dyn_cast<BinaryOperator>(thenBodyStmt)) {
      ...
      if (const VarDecl *varDecl = dyn_cast_or_null<VarDecl>(declStmt->getSingleDecl())) {
        if (checkThenBodyStmt(varDecl->getAnyInitializer(), condReceiver, condSel))
          return true;
      }
    }
    return false;
  ...
  void WalkAST::checkToplevelThenBodyStmt(const Stmt &thenStmt, const Expr &condReceiver, const Selector &condSel) const {
    for (auto it = thenStmt.child_begin(), end = thenStmt.child_end(); it != end; ++it) {
      if (*it) {
  ...
  void WalkAST::checkIfStmt(const IfStmt &ifStmt) const {
    ...
    if (condMsgExpr && isRespondsToSelector(condMsgExpr->getSelector())) {
      ...
      if (!condReceiver) return;
      const Expr *condArgument = condMsgExpr->getArg(0);
      if (const ObjCSelectorExpr *condSelExpr = dyn_cast_or_null<ObjCSelectorExpr>(condArgument)) {
...
public:
  ...
  void checkASTCodeBody(const Decl *decl, AnalysisManager& am, BugReporter &br) const {
