...
// TODO: use code-annotations instead of a hardcoded list of pseudo-init methods??
// TODO: try to use clang symbols to track references to (pseudo)ivars and support aliasing, use the 'assign' callback instead of catching BinOp stmts, use the deadsymbols callback
...
namespace DanglingDelegate {
  ...
  struct IvarDynamicState {
    StringSet _assignPropertyWasCleared; // e.g. 'delegate' would be in the set if the ivar _x has processed [_x setDelegate:nil] in the code
    ...
    IvarDynamicState() :
    _targetWasCleared(false),
    _observerWasCleared(false)
    { }
    ...
    IvarDynamicState(const IvarDynamicState *rhs) :
    _assignPropertyWasCleared(rhs ? rhs->_assignPropertyWasCleared : StringSet()),
...
REGISTER_MAP_WITH_PROGRAMSTATE(IvarMap, const ObjCIvarDecl *, DanglingDelegate::IvarDynamicState)

// hack to allow setting the initial state to a non default value at the beginning of trace
...
namespace DanglingDelegate {
  ...
  class DanglingDelegateChecker : public Checker <
  check::ASTDecl<ObjCImplDecl>,
  ...
  >
  {
    ...
    const ObjCImplFacts *getCurrentFacts(const ObjCInterfaceDecl *topInterface) const;

    // verify that the given ivar (occuring in expr) is ok to be cleared
    void verifyIvarDynamicStateAgainstStaticFacts(const Expr &expr, const ObjCIvarDecl *ivarDecl, CheckerContext &context) const;

    // pseudo-init methods have a different state where all ivars are considered cleared
  ...
  public:
    ...
    void checkASTDecl(const ObjCImplDecl *implDecl, AnalysisManager &manager, BugReporter &bugReporter) const;

    // clear the syntactic facts at the end
    void checkEndOfTranslationUnit(const TranslationUnitDecl *tuDecl, AnalysisManager &manager, BugReporter &bugReporter) const;

    void checkEndFunction(CheckerContext &context) const;

    void checkPreStmt(const Stmt *stmt, CheckerContext &context) const;

    void checkPostObjCMessage(const ObjCMethodCall &message, CheckerContext &context) const;
  ...
  DanglingDelegateChecker::DanglingDelegateChecker() {
    _bugType.reset(new BugType(this,
                               "Leaking unsafe reference to self",
                               "Memory error (Facebook)"));
    _bugType->setSuppressOnSink(true);
  }

  void DanglingDelegateChecker::checkEndOfTranslationUnit(const TranslationUnitDecl *tuDecl,
  ...
  const ObjCImplFacts *DanglingDelegateChecker::getCurrentFacts(const ObjCInterfaceDecl *topInterface) const {
    if (!topInterface) {
      return NULL;
    }
...
//    }
  ...
  // Tests the opcode
  static bool matchOpcode(BinaryOperator::Opcode op, bool assumption, bool wantEquality)
  {
    switch (op) {
      case BO_EQ: return (assumption == wantEquality);
      case BO_NE: return (assumption != wantEquality);
  ...
  static bool matchConditionWithComparison(const SVal &cond,
    ...
    const SymSymExpr *sse = dyn_cast_or_null<SymSymExpr>(cond.getAsSymbolicExpression());
    if (!sse || !matchOpcode(sse->getOpcode(), assumption, wantEquality)) {
      // abort
      return false;
    }

    // Tries to matches the instances of A and B in the lhs and rhs symbolic expressions
  ...
  static bool matchConditionWithIntComparison(const SVal &cond,
    ...
    const SymIntExpr *sie = dyn_cast_or_null<SymIntExpr>(cond.getAsSymbolicExpression());
    if (sie) {
      // Tries to matches the instances of A and B in the lhs and rhs symbolic expressions
      ...
      const IntSymExpr *ise = dyn_cast_or_null<IntSymExpr>(cond.getAsSymbolicExpression());
      if (ise) {
        // Tries to matches the instances of A and B in the lhs and rhs symbolic expressions
  ...
  //
  // In practice, the internal representations look like this: (as displayed by cond.dump())
  //   (conj_$5{Test *}) != (reg_$0<self>)
  static bool matchConditionWithInterestingDelegateNotEqualToSelf(const SVal &cond,
                                                                  bool assumption,
                                                                  const ObjCImplFacts &facts,
                                                                  const ObjCIvarDecl **foundIvar,
                                                                  const ObjCPropertyDecl **foundProperty) {
    ...
    if (!matchConditionWithComparison<SymbolConjured, SymbolRegionValue>(cond, assumption, false, &symConj, &symRVal)) {
      // abort
      return false;
    }
    assert(symConj && symRVal);

    // We are looking for an assertion that ObjCMessageExprReadingAnIvarProperty != self
    // first look for  'self'
    const DeclRegion *declRegion = dyn_cast_or_null<DeclRegion>(symRVal->getRegion());
    if (!declRegion) {
      return false;
    }
    const ImplicitParamDecl *decl = dyn_cast_or_null<ImplicitParamDecl>(declRegion->getDecl());
    // FIXME: do not use the name?
    if (!decl || decl->getNameAsString() != "self") {
      return false;
    }

    const ObjCMessageExpr *expr = dyn_cast_or_null<ObjCMessageExpr>(symConj->getStmt());
    return (expr ? matchObjCMessageExprForInterestingDelegate(*expr, facts, foundIvar, foundProperty) : false);
  }
  ...
  // In practice, the internal representations look like this:  (as displayed by cond.dump())
  //   (conj_$5{Test *}) == 0U
  static bool matchConditionWithInterestingDelegateEqualToNil(const SVal &cond,
                                                              bool assumption,
                                                              const ObjCImplFacts &facts,
                                                              const ObjCIvarDecl **foundIvar,
                                                              const ObjCPropertyDecl **foundProperty) {
    ...
    const llvm::APSInt *val = NULL;
    if (!matchConditionWithIntComparison<SymbolConjured>(cond, assumption, true, &symConj, &val)) {
      return false;
     }
    assert(symConj && val);
    if (*val != 0) {
  ...
  // In practice, the internal representations look like this:  (as displayed by cond.dump())
  //   (conj_$4{Worker *}) == 0U
  static bool matchConditionWithInterestingPropertyEqualToNil(const SVal &cond,
                                                              bool assumption,
                                                              const ObjCImplFacts &facts,
                                                              const ObjCIvarDecl **foundIvar) {
    ...
    if (!ivarDecl) {
      ...
      const ObjCPropertyDecl *propDecl = matchObjCMessageWithPropertyGetter(*msgExpr);
      if (!propDecl) {
        return false;
      }
  ...
  static bool matchConditionWithInterestingIvarEqualToNil(const SVal &cond,
                                                          bool assumption,
                                                          const ObjCImplFacts &facts,
                                                          const ObjCIvarDecl **foundIvar) {
    ...
    if (matchConditionWithIntComparison<SymbolRegionValue>(cond, assumption, true, &sym, &val)) {
      assert(sym && val);
      ivarReg = dyn_cast_or_null<ObjCIvarRegion>(sym->getRegion());
    } else {
      const SymbolDerived *symD = NULL;
      if (matchConditionWithIntComparison<SymbolDerived>(cond, assumption, true, &symD, &val)) {
        assert(symD && val);
  ...
  // Here we want to intercept tests that restrict the current branch to "favorable cases"
  // We then mark the state accordingly in a way that will survive to whatever happens next in the trace.
  // This is unsound but for this trace at least the programmer seems to know what he/she is doing.
  ProgramStateRef DanglingDelegateChecker::evalAssume(ProgramStateRef state,
                                                      const SVal &cond,
                                                      bool assumption) const
  {
    const ObjCInterfaceDecl *currentInterface = (const ObjCInterfaceDecl *)state->get<CurrentObjCInterfaceDecl>();
    ...
    if (matchConditionWithInterestingDelegateNotEqualToSelf(cond, assumption, *facts, &ivarDecl, &propDecl)
        || matchConditionWithInterestingDelegateEqualToNil(cond, assumption, *facts, &ivarDecl, &propDecl))
    {
    ...
    if (matchConditionWithInterestingPropertyEqualToNil(cond, assumption, *facts, &ivarDecl)
        || matchConditionWithInterestingIvarEqualToNil(cond, assumption, *facts, &ivarDecl)) {
      assert(ivarDecl);
      IvarDynamicState clearedStateForIvar(facts->_ivarFactsMap.at(ivarDecl));
...
// needed because REGISTER_CHECKER_IN_PLUGIN computes function identifiers with the given name
using namespace DanglingDelegate;

REGISTER_CHECKER_IN_PLUGIN(DanglingDelegateChecker,
